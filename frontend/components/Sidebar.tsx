...
export default function Sidebar({ registry, onAddNode, onClear, stats }: SidebarProps) {
    ...
    return (
        <div className="w-96 flex-shrink-0 border-r border-slate-700 bg-slate-900 flex flex-col shadow-2xl relative z-20">
            {/* Header - SOLID background */}
            <div className="p-6 border-b border-slate-700 bg-slate-800">
                <div className="flex items-center gap-3 mb-4">
                    <div className="p-2 rounded-lg bg-gradient-to-br from-blue-500 to-emerald-500 shadow-lg">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Physics-Informed Neural Networks</p>
                    </div>
                </div>
                ...
                <div className="relative mt-4">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-600 rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    />
                    {searchTerm && (
                        <button
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${!selectedCategory
                                ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/30'
                                : 'bg-slate-950 text-slate-400 hover:bg-slate-800 border border-slate-700'
                            }`}
                    >
                        All
                    </button>
                ...
                {Object.entries(filteredCategories).map(([category, nodeTypes]) => (
                    <div key={category}>
                        <button
                            ...
                            className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-slate-800 transition-colors group mb-2"
                        >
                                ...
                                {nodeTypes.map((type) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="group flex items-center gap-3 p-3 rounded-xl bg-slate-800 border border-slate-700 hover:border-blue-500 hover:bg-slate-750 hover:shadow-lg hover:shadow-blue-500/20 transition-all text-left transform hover:scale-[1.02] active:scale-[0.98]"
                                        >
                                            <div className="p-2.5 rounded-lg bg-slate-950 border border-slate-700 text-blue-400 group-hover:text-blue-300 group-hover:border-blue-500 group-hover:scale-110 transition-all shadow-lg">
                                                <Icon size={18} />
                                            </div>
            ...
            {/* Footer Stats - SOLID background */}
            <div className="p-4 border-t border-slate-700 bg-slate-800">
                <div className="flex items-center justify-between text-xs text-slate-400">
                    <div className="flex items-center gap-4">
                        <span className="font-medium">{stats.nodes} Nodes</span>
                        <span className="font-medium">{stats.edges} Connections</span>
                    </div>
                    <button
                        onClick={onClear}
                        className="text-red-400 hover:text-red-300 hover:bg-red-500/10 px-3 py-1.5 rounded-lg transition-all font-medium"
                    >
                        Clear
                    </button>
                </div>
