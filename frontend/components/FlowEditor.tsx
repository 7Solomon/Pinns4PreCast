    ...
    const handleLoadSubmit = async (filename: string) => {
        try {
            ...
            const res = await axios.get(
                `http://localhost:8000/graphs/load/${filename}`
            );
    ...
    return (
        <>
            <div className="flex flex-col w-full h-full bg-slate-950">
                {/* Top bar: tab switch + controls */}
                <div className="flex items-center justify-between px-6 py-3 border-b border-slate-800 bg-slate-900/90 backdrop-blur">
                    {/* Left: Editor / Monitor switch */}
                    ...
                    {/* Right: controls (only relevant in editor mode) */}
                    {activeTab === "editor" && (
